...
Cursor Share 支持**英文**（默认）和**中文**。语言会根据系统区域设置自动检测。所有 UI 元素、对话框、通知和托盘菜单均已翻译。
...
**Q: macOS 上提示「应用已损坏」或「无法打开」？**
A: 这是 macOS Gatekeeper 隔离机制导致的。在终端中运行以下命令即可解决：
```bash
sudo xattr -r -d com.apple.quarantine /Applications/Cursor\ Share.app
```
如果应用不在 `/Applications` 目录下，请替换为实际路径。
